...
from gdeep.smale_quiver.utility import *
...
class SmaleQuiverComputer:
    ...
    def fit(self, net, input_shape, n_sample = int(200)):

        ''' Computes the smale quiver of the fonction encoded by class_nn
        Stores it in the attribute .quiver

        Args:
            net (nn.module) : a nn.module encoding the function to study
        ...
        min_max = get_min_max(net, input_shape, self.n_epochs, n_sample)
        self.quiver = build_graph(min_max, self.clusterer)
        add_index(self.quiver, net, threshold= 0.1)
    ...
    def fit_transform(self, class_nn, input_shape, n_sample = int(200)):
        ''' Computes the smale quiver of the function encoded by class_nn
